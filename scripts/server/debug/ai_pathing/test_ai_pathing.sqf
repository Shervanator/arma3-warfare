...
if (assert (isServer and !isDedicated)) then {
  ...
  private _handler = addMissionEventHandler ["MapSingleClick", {
    ...
    if (kyf_WG_debug_aiPath_waiting) then {
      ...
      hint "Path chosen. Displaying predefined path of best fit...";
      ...
      private _path = [getMarkerPos kyf_WG_debug_aiPath_startPoint, getMarkerPos kyf_WG_debug_aiPath_endPoint] call kyf_WF_getPath;
