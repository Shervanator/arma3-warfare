...
Parameters:
 ...
 (optional) _allowWater: Boolean. If true water paths will also be considered, while if false only land paths will be considered. If not specidifed, true by default
 ...
 2: Bool. If true then the path contains water and if false it does not.
...
private ["_startPos", "_endPos", "_allowWater", "_startZoneInfo", "_endZoneInfo", "_startZone", "_endZone", "_hashTable", "_bestPath"];
params ["_startPos", "_endPos", "_allowWater", "_startZoneInfo", "_endZoneInfo"];

//------------------------------------------------------------------------------
// Check state of optional variables

// Water pathing
if (isNil "_allowWater") then {
  // Water pathing is enabled by default
  _allowWater = true;
};
...
if !((_startZone == _endZone) or (_startZone == -1) or (_endZone == -1)) then {  /*If the destination and start pos are both in the same zone, or if either the _startZone or _endZone are undefined (kyf_WF_findZone returned -1),
  ...
  // path format: [path identifier number, current pos, [[path positions], [path names]], current zone, total path cost so far, distSqr form endPos, path cost + remaining dist, bool representing if the path contains water zones]
  // [path names] are only kept for debug purposes at the momnet, so that a log of the shortest path can be read
  private _pathQue = [[-1, _startPos, [[_startPos], ["start"]], _startZone, 0, 0, 0, false]];
  ...
  while {(count _pathQue) > 0} do {
    ...
    // Check which paths the exit points in the zone lead to and create new paths for _pathQue
    {
      ...
      if !(_zepIndex isEqualTo _curentPosIndex) then { // Make sure that we are not backtracking through the path we already came from
        ...
        private _linkZoneIsWater = (kyf_WG_allZones select _linkZone) select 4;

        // Check water zone conditions
        if !(_linkZoneIsWater and !_allowWater) then {
          // Either water zones are allowed, or this target zone is not a target zone
          ...
          if (_addPath) then {
            ...
            // If true, then this path involves crossing water, and if false the it is purely a land path
            private _isWaterPath = _currentPath select 7;

            // If this path requires travelling through water, make sure to flag the path as such
            if (_linkZoneIsWater) then {
              _isWaterPath = true;
            };
            ...
            if (_linkZone == _endZone) then { // Does this path lead us to the target zone?
              ...
              _bestPath = [_pathHistory, _totalCost, _isWaterPath];
              ...
              [_pathQue, [_linkIndex, _linkPos, _pathHistory, _linkZone, _pathCost, _remainingDist, _totalCost, _isWaterPath], _rank] call kyf_WF_pushBackToIndex;
            };
          };
        };
      };
...
if (_bestPath isEqualTo []) then {
  /* The default return vale, as a straight path between startPos and endPos. Occurs if either pos supplied is not in a zone, both pos are in the same zone, 
  or a path cannot be generated between the positions using zones */
  _bestPath = [[[_startPos, _endPos], ["start", "end"]], _startPos distanceSqr _endPos, false];
};
...
/* Return format: [full path, total path cost, is water], where full path is an array of all positions, and is water is a bool representing water - if true then 
water zones must be crossed to reach the target and if false it is a purely land path */
[(_bestPath select 0) select 0, _bestPath select 1, _bestPath select 2]
